...
def test_get_clan_tag_endpoint_success():
    responses.add(responses.GET, ID_LOOKUP_BASE_URL.format(api_key="dummy", clan_tag="EXAMPLE"),
                  json={"status": "ok", "meta": {"count": 1, "total": 1}, "data":
                        [{"clan_id": 5000000, "tag": "EXAMPLE"}]}, status=200)
...
def test_get_clan_tag_endpoint_api_error():
    responses.add(responses.GET, ID_LOOKUP_BASE_URL.format(api_key="dummy", clan_tag="EXAMPLE"),
                  json={"status": "error", "error": {"message": "TEST_API_ERROR"}}, status=200)
...
def test_get_clan_tag_endpoint_api_not_found():
    responses.add(responses.GET, ID_LOOKUP_BASE_URL.format(api_key="dummy", clan_tag="EXAMPLE"),
                  json={"status": "ok", "data": []}, status=200)
...
def test_get_clan_id_endpoint_success():
    responses.add(responses.GET, TAG_LOOKUP_BASE_URL.format(api_key="dummy", clan_id=5000000),
                  json={"status": "ok", "meta": {"count": 1}, "data": {"5000000": {"tag": "EXAMPLE"}}}, status=200)
...
def test_get_clan_id_endpoint_api_not_found():
    responses.add(responses.GET, TAG_LOOKUP_BASE_URL.format(api_key="dummy", clan_id=5000000),
                  json={"status": "ok", "data": {"5000000": {"tag": None}}}, status=200)
